...
export default function getConsoleOutput(
  ...
  const logEntries = buffer.reduce((output, {type, message, origin}) => {
    ...
    if (type === 'warn') {
      message = JestLogger.yellow(message);
      typeMessage = JestLogger.yellow(typeMessage);
      noStackTrace = globalConfig?.noStackTrace ?? false;
      noCodeFrame = false;
    } else if (type === 'error') {
      message = JestLogger.red(message);
      typeMessage = JestLogger.red(typeMessage);
      noStackTrace = globalConfig?.noStackTrace ?? false;
      noCodeFrame = false;
    }
    ...
    return `${
      output + TITLE_INDENT + JestLogger.dim(typeMessage)
    }\n${message.trimEnd()}\n${JestLogger.dim(formattedStackTrace.trimEnd())}\n\n`;
  }, '');
