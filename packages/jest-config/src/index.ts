...
const ensureNoDuplicateConfigs = (
  ...
  for (const config of parsedConfigs) {
    ...
    if (configPathMap.has(configPath)) {
      const message = `Whoops! Two projects resolved to the same config path: ${JestLogger.bold(
        String(configPath),
      )}:

  Project 1: ${JestLogger.bold(projects[parsedConfigs.indexOf(config)])}
  Project 2: ${JestLogger.bold(
    projects[parsedConfigs.indexOf(configPathMap.get(configPath))],
  )}

This usually means that your ${JestLogger.bold(
        '"projects"',
