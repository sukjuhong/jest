...
export function createReporterError(
  ...
  const errorMessage =
    ...
    `    ${JestLogger.bold.green(validReporterTypes.join(' or '))}\n` +
    '  but instead received:\n' +
    `    ${JestLogger.bold.red(getType(reporterValue))}`;
...
export function createArrayReporterError(
  ...
  const errorMessage =
    ...
    '  Expected:\n' +
    `    ${JestLogger.bold.red(expectedType)}\n` +
    '  Got:\n' +
    `    ${JestLogger.bold.green(getType(value))}\n` +
