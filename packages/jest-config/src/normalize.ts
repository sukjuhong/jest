  ...
  try {
    ...
    if (!rootStat.isDirectory()) {
      throw createConfigError(
        `  ${JestLogger.bold(path)} in the ${JestLogger.bold(
          key,
    ...
    if (error.code === 'ENOENT') {
      throw createConfigError(
        `  Directory ${JestLogger.bold(path)} in the ${JestLogger.bold(
          key,
    ...
    throw createConfigError(
      `  Got an error trying to find ${JestLogger.bold(path)} in the ${JestLogger.bold(
        key,
...
const setupPreset = async (
  ...
  try {
    ...
    if (error instanceof SyntaxError || error instanceof TypeError) {
      throw createConfigError(
        `  Preset ${JestLogger.bold(presetPath)} is invalid:\n\n  ${
          error.message
        }\n  ${error.stack}`,
      );
    }
    ...
    if (error.message.includes('Cannot find module')) {
      if (error.message.includes(presetPath)) {
        ...
        if (preset) {
          throw createConfigError(
            `  Module ${JestLogger.bold(
              presetPath,
            )} should have "jest-preset.js" or "jest-preset.json" file at the root.`,
          );
        }
        throw createConfigError(
          `  Preset ${JestLogger.bold(presetPath)} not found relative to rootDir ${JestLogger.bold(options.rootDir)}.`,
        );
      }
      throw createConfigError(
        `  Missing dependency in ${JestLogger.bold(presetPath)}:\n\n  ${
          error.message
        }\n  ${error.stack}`,
      );
    }
    ...
    throw createConfigError(
      `  An unknown error occurred in ${JestLogger.bold(presetPath)}:\n\n  ${
        error.message
      }\n  ${error.stack}`,
    );
  }
...
const normalizeRootDir = (
  ...
  if (!options.rootDir) {
    throw createConfigError(
      `  Configuration option ${JestLogger.bold('rootDir')} must be specified.`,
    );
  }
...
function printConfig(opts: Array<string>) {
  ...
  return JestLogger.bold(`extensionsToTreatAsEsm: [${string}]`);
}
...
function validateExtensionsToTreatAsEsm(
  ...
  if (extensionWithoutDot) {
    throw createConfigError(
      ...
      )} includes a string that does not start with a period (${JestLogger.bold(
        '.',
      )}).
  ...
  if (extensionsToTreatAsEsm.includes('.js')) {
    throw createConfigError(
      `  Option: ${printConfig(extensionsToTreatAsEsm)} includes ${JestLogger.bold(
        "'.js'",
      )} which is always inferred based on ${JestLogger.bold(
        'type',
      )} in its nearest ${JestLogger.bold('package.json')}.`,
    );
  }
...
export default async function normalize(
  ...
  optionKeys.reduce((newOptions, key: keyof Config.InitialOptions) => {
    ...
    switch (key) {
      ...
      case 'moduleFileExtensions': {
        ...
        if (
          ...
          const errorMessage =
            ...
            `    ${JestLogger.bold.red(JSON.stringify(value))}`;
      ...
      case 'displayName': {
        ...
        if (typeof displayName === 'object') {
          ...
          if (
            ...
            const errorMessage =
              `  Option "${JestLogger.bold('displayName')}" must be of type:\n\n` +
              '  {\n' +
              '    name: string;\n' +
              '    color: string;\n' +
              '  }\n';
      ...
      case 'testTimeout': {
        if (oldOptions[key] < 0) {
          throw createConfigError(
            `  Option "${JestLogger.bold('testTimeout')}" must be a natural number.`,
          );
        }
  ...
  if (newOptions.testRegex.length > 0 && options.testMatch) {
    throw createConfigError(
      `  Configuration options ${JestLogger.bold('testMatch')} and` +
        ` ${JestLogger.bold('testRegex')} cannot be used together.`,
    );
  }
