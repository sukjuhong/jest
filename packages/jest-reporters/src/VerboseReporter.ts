...
export default class VerboseReporter extends DefaultReporter {
  ...
  private _getIcon(status: string) {
    if (status === 'failed') {
      return JestLogger.red(ICONS.failed);
    } else if (status === 'pending') {
      return JestLogger.yellow(ICONS.pending);
    } else if (status === 'todo') {
      return JestLogger.magenta(ICONS.todo);
    } else {
      return JestLogger.green(ICONS.success);
    }
  }
  ...
  private _logTest(test: AssertionResult, indentLevel: number) {
    ...
    const time = test.duration
      ...
      : '';
    this._logLine(
      `${status} ${JestLogger.dim(test.title + time)}`,
      indentLevel,
    );
  }
  ...
  private _logTodoOrPendingTest(indentLevel: number) {
    return (test: AssertionResult): void => {
      ...
      const text = JestLogger.dim(`${printedTestStatus} ${test.title}`);
